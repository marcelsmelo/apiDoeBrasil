...
module.exports = {
  ...
  cadastrar: async (req, res, next) => {
    ...
    try {
      ...
      res
        .status(500)
        .json({ msg: "Erro ao cadastrar o usuário!", error: error.message });
    }
  },
  ...
  login: (req, res, next) => {
    ...
    Usuario.findOne({
      ...
      .then((user) => {
        if (!user) {
          //Usuário (cpfCnpj) não foi encontrado
          res.status(500).json({
          ...
          if (user.comparePassword(req.body.senha)) {
            ...
            user
              .generateAuthToken(req.body.loginType) //Gerar o token JWT
              .then((sucesso) => {
  ...
  editar: async (req, res, next) => {
    try {
      ...
      res
        .status(500)
        .json({ msg: "Erro ao editar usuário", error: error.message });
    }
  },
  ...
  remover: async (req, res, next) => {
    try {
      ...
      return res
        .status(500)
        .json({ msg: "Erro ao remover Usuário!", error: error.message });
    }
  },
  ...
  selecionarComoParceiro: async (req, res, next) => {
    try {
      ...
      res
        .status(200)
        .send({ msg: "Usuário transformado em Usuário-Parceiro com sucesso" });
    } catch (error) {
      res
        .status(500)
        .send({ msg: "Erro ao atualizar usuário!", error: error.message });
    }
  },
  ...
  removerComoParceiro: async (req, res, next) => {
    try {
      ...
      res
        .status(200)
        .send({ msg: "Usuário não está mais vinculado ao Parceiro!" });
    } catch (error) {
      res
        .status(500)
        .send({ msg: "Erro ao atualizar usuário!", error: error.message });
  ...
  usuariosParceiro: async (req, res, next) => {
    ...
    try {
      ...
      res
        .status(500)
        .send({ msg: "Erro ao buscar usuários!", error: error.message });
    }
  },
  ...
  buscarParceiros: async (req, res, next) => {
    ...
    try {
      ...
      res.status(500)
        .json({ msg: "Erro ao buscar parceiros", error: error.message });
    }
  },

  //Verifica se o e-mail já está cadastrado
  hasEmail: async (req, res, next) => {
    let email = req.query.email;
    try {
      let user = await Usuario.findOne({where: {'email': email}});
      if (user != null) return res.status(200).json({ msg: "E-mail já cadastrado no sistema." });
      else return res.status(204).json({ msg: "E-mail não encontrado no sistema." });
    } catch (e) {
      res.status(500)
        .json({ msg: "Erro ao verificar e-mail. Tente novamente.", error: error.message });
    }
  },

  //Verifica se o CPF ou CNPJ já foi utilizado em algum cadastro
  hasCpfCnpj: async (req, res, next) => {
    let cpfCnpj = req.query.cpfCnpj;
    try {
      let user = await Usuario.findOne({where: {'cpfCnpj': cpfCnpj}});
      if (user != null) return res.status(200).json({msg: "CPF ou CNPJ já cadastrado no sistema." });
      else return res.status(204).json({msg: "CPF ou CNPJ não encontrado no sistema." });
    } catch (e) {
      res.status(500)
        .json({ msg: "Erro ao verificar Usuário com CPF ou CNPJ informado. Tente novamente.", error: error.message });
    }
  },

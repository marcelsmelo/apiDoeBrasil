...
module.exports = {
   ...
   buscarPedido: async (req, res, next) => {
      ...
      if (req.query.id) condition.id = req.query.id;
         ...
         //Em caso de BuscarTodos, busca os pedidos vinculados aos usuários e/ou Parceiros logados

         if (req.user.loginType == "U") condition.usuarioId = req.user.id;
         //Usuário comum - Busca todos pedidos do usuário
         else if (req.user.loginType == "P")
            condition.parceiroId = req.user.parceiroId; //Parceiro - Busca todos pedidos associados ao parceiro
      ...
      try {
            ...
            include: [
               {
                  ...
                  on: {
                     id: Sequelize.where(
                        Sequelize.col("pedido.usuarioId"),
                        "=",
                        Sequelize.col("usuario.id")
                     ),
                  },
               ...
               },
               {
                  ...
                  on: {
                     id: Sequelize.where(
                        Sequelize.col("pedido.parceiroId"),
                        "=",
                        Sequelize.col("parceiro.id")
                     ),
                  },
               },
            ],
         ...
         res
            .status(500)
            .json({ msg: "Erro ao buscar pedidos", error: error.message });
      }
   },
   ...
   cadastrar: async (req, res, next) => {
      ...
      if (nroPedidos > 0) {
         //Não permitir a criação de um novo pedido enquanto existir pedido em aberto
         return res
            .status(500)
            .json({
               msg: "Erro ao cadastrar pedido",
               error: "O usuário já possui pedido em aberto!",
            });
      }

      //Verificar se o usuário solicitou pelo menos um material
      if (
         !req.body.generoAlimenticio &&
         !req.body.higienePessoal &&
         !req.body.artigoLimpeza &&
         !req.body.outros
      ) {
         return res
            .status(500)
            .json({
               msg: "Erro ao cadastrar pedido",
               error: "Solicite pelo menos um material!",
            });
      }
      ...
      if (req.user.loginType == "U") {
         if (!req.body.parceiroId)
            //Ao solicitar ajuda, o usuário deve selecionar um Parceiro
            return res
               .status(500)
               .json({
                  msg: "Erro ao cadastrar pedido",
                  error: "O Pedido deve ser vinculado a um Parceiro!",
               });
         ...
         if (!req.body.usuarioId)
            return res
               ...
               .json({
                  msg: "Erro ao cadastrar pedido",
                  error: "O Pedido deve ser vinculado a um usuário!",
               });
      ...
      if (req.user.group == "P") {
         //Se for parceiro, vincular o pedido a um usuário cadastrado
      ...
      try {
         ...
         res
            .status(500)
            .json({ msg: "Erro ao cadastrar pedido", error: error.message });
      }
   },
      ...
      try {
         ...
         if (!pedido)
            return res
               ...
               .json({
                  msg:
                     "Alteração não autorizada! O usuário logado não tem permissão para realizar essa operação",
                  error: null,
               });
         ...
         if (pedido.status == 0) {
            ...
            if (req.user.loginType == "U")
               //O usuário pode alterar o parceiro do pedido.
            ...
            throw new Error(
               "Alteração não autorizada! O pedido já foi selecionado para entrega ou finalizado"
            );
         }
      } catch (error) {
         res
            .status(500)
            .json({ msg: "Erro ao editar pedido", error: error.message });
      }
   },
   ...
   remover: async (req, res, next) => {
      ...
      if (pedido.status != 0)
         return res
            ...
            .json({
               msg:
                  "Alteração não autorizada! Pedido já selecionado para atendimento!",
               error: null,
            });
      ...
      try {
         ...
         res
            .status(500)
            .json({ msg: "Erro ao remover pedido!", error: error.message });
      }
   },
      ...
      Pedido.update(
         ...
         .then((success) => {
            if (success[0])
               ...
               throw new Error(
                  "Alteração não autorizada! O usuário logado não tem permissão ou o pedido já está em aberto."
               );
         })
         .catch((error) => {
            res
   ...
   verificarPedidoAberto: async (req, res, next) => {
      ...
      if (!nroPedidos)
         res.status(200).json({ msg: `O usuário não possui pedidos em aberto!` });
      else
         res.status(500).json({ msg: `O usuário possui ${nroPedidos} abertos!` });
   },
};
